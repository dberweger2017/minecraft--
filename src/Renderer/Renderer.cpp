...
Renderer::Renderer(const RendererInitInfo& info) : window(info.window) {
...
void Renderer::createSwapChain() {
    // Simplified for refactor (assumes standard support)
...
void Renderer::createGraphicsPipeline() {
    // Note: We need to use the binary dir path for shaders
    auto vertShaderCode = readFile(std::string(MINECRAFT_VK_SHADER_DIR) + "/square.vert.spv");
    auto fragShaderCode = readFile(std::string(MINECRAFT_VK_SHADER_DIR) + "/square.frag.spv");
...
void Renderer::updateUniformBuffer(uint32_t currentImage, const Camera& camera, glm::vec3 sunDirection, glm::vec3 sunColor) {
...
ChunkMesh Renderer::createChunkMesh(const std::vector<Vertex>& vertices) {
    ChunkMesh mesh{};
    ...
    void* data;
    if (vkMapMemory(device, stagingBufferMemory, 0, bufferSize, 0, &data) != VK_SUCCESS) throw std::runtime_error("failed to map memory!");
...
void Renderer::destroyChunkMesh(ChunkMesh& mesh) {
    if (mesh.vertexBuffer != VK_NULL_HANDLE) {
